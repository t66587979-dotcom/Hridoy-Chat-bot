...
module.exports.config = {
  ...
  version: "1.0.2",
  hasPermssion: 0,
  credits: "rX (Improved by Grok)",
  description: "র‍্যান্ডম জাপানি গার্ল PP (profile picture)",
...
module.exports.run = async function ({ api, event }) {
  ...
  const cachePath = path.join(__dirname, "cache", `japan_${Date.now()}.jpg`);

  try {
    // Loading message
    await api.sendMessage("জাপানি গার্ল লোড হচ্ছে... 😍💕", threadID, messageID);

    const response = await axios.get(randomLink, { responseType: "arraybuffer", timeout: 15000 });

    fs.writeFileSync(cachePath, Buffer.from(response.data));

    await api.sendMessage({
      body: `𝗚𝗮́𝗶 𝗡𝗵𝗮̣̂𝘁 𝗡𝗲̀ 𝗠𝗲̂̀ 𝗫𝗮́ 𝗟𝗮̀ 𝗛𝗲̂́ 💕\nসোর্স: Imgur\nটোটাল ছবি: ${links.length}\n\nআরেকটা চাইলে আবার !japan দাও 😘`,
      attachment: fs.createReadStream(cachePath)
    }, threadID, () => fs.unlinkSync(cachePath), messageID);

  } catch (error) {
    console.error("Japan image error:", error.message);
    api.sendMessage("ছবি লোড করতে সমস্যা হয়েছে 😔 (লিঙ্ক মরে গেছে বা নেটওয়ার্ক ইস্যু)। আবার ট্রাই করো!", threadID, messageID);
  }
};